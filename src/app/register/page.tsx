...
import { CATEGORIES, CERT_COLORS } from '@/lib/data';
                ...
                <div className="flex flex-wrap gap-2">
                  {CERTS.map(cert => {
                    ...
                    return (
                      <button
                        key={cert}
                        onClick={() => set('certs', selected ? form.certs.filter(c => c !== cert) : [...form.certs, cert])}
                        className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-colors ${selected ? CERT_COLORS[cert] + ' border-transparent' : 'bg-white border-gray-200 text-gray-500 hover:border-gray-300'}`}
                      >
                        {selected ? '✓ ' : ''}{cert}
                      </button>
                    );
          ...
          {step === 4 && (
            <>
              ...
              <div className="bg-gray-50 rounded-xl p-4 space-y-3 text-sm">
                ...
                {form.certs.length > 0 && <Row label="보안 인증" value={form.certs.join(', ')} />}
              </div>
