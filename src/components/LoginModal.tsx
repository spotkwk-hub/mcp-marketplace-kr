          ...
          {mode !== 'forgot' && (
            <div className="flex bg-gray-100 rounded-xl p-1 mb-6">
              {(['login', 'signup'] as const).map(m => (
                <button key={m} onClick={() => setMode(m)} className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-colors ${mode === m ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                  {m === 'login' ? '로그인' : '회원가입'}
                </button>
              ))}
            </div>
            ...
            <>
              ...
              {mode !== 'forgot' && (
                <div className="space-y-2.5 mb-5">
                  {SOCIAL.map(s => (
                    <button key={s.id} className={`w-full flex items-center justify-center gap-2.5 py-2.5 rounded-xl text-sm font-semibold transition-opacity hover:opacity-90 ${s.bg} ${s.text}`}>
              ...
              <form onSubmit={handleSubmit} className="space-y-3.5">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300"
                      />
                ...
                <button
                  ...
                  className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white rounded-xl text-sm font-semibold transition-colors mt-1"
                >
