...
const server = new Server({ name: 'g2b-mcp', version: '0.1.0' }, { capabilities: { tools: {} } });
const API_KEY = process.env.G2B_API_KEY ?? '';
const BASE = 'https://apis.data.go.kr/1230000';
async function g2bFetch(path, params = {}) {
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    try {
        if (!API_KEY)
            throw new Error('G2B_API_KEY 환경 변수가 설정되지 않았습니다.');
        if (name === 'search_bid_announcements') {
            const data = await g2bFetch('/ad/bidpblancList', {
                inqryDiv: '1', inqryBgnDt: args.start_date ?? '',
                inqryEndDt: args.end_date ?? '',
                bidNtceNm: args.keyword,
                numOfRows: args.num_of_rows ?? '10',
            });
            return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
        }
        if (name === 'get_bid_award_info') {
            const data = await g2bFetch('/ad/scsbidList', {
                bidNtceNo: args.bid_no, bidNtceOrd: args.bid_seq ?? '00',
            });
            return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
        }
        if (name === 'get_contract_info') {
            const data = await g2bFetch('/cn/cntrctInfoList', { cntrctNo: args.contract_no });
            return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
        }
        if (name === 'get_pre_announcement') {
            const data = await g2bFetch('/ad/preAnncmntList', {
                prdctClsfcNoNm: args.keyword,
                inqryBgnDt: args.start_date ?? '',
                inqryEndDt: args.end_date ?? '',
            });
            return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
        }
        if (name === 'get_user_info') {
            const data = await g2bFetch('/us/usrInfoList', { bizno: args.biz_no });
            return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
        }
