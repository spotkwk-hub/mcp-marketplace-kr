...
const server = new Server(
  { name: 'douzone-icube-mcp', version: '0.1.0' },
  { capabilities: { tools: {} } }
);
...
async function dFetch(path: string, params: Record<string, string> = {}) {
  ...
  if (!res.ok) throw new Error(`더존 iCUBE API 오류: ${res.status} ${res.statusText}`);
  return res.json();
}
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  try {
    if (!API_KEY)    throw new Error('DOUZONE_API_KEY 환경 변수가 설정되지 않았습니다.');
    if (!COMPANY_CD) throw new Error('DOUZONE_COMPANY_CD 환경 변수가 설정되지 않았습니다.');

    if (name === 'get_vouchers') {
      const data = await dFetch('/accounting/vouchers', {
        voucherType: args.voucher_type as string,
    ...
    if (name === 'get_financial_summary') {
      const data = await dFetch('/finance/summary', {
        reportType: args.report_type as string, fiscalYear: args.fiscal_year as string,
    ...
    if (name === 'get_sales_purchase') {
      const data = await dFetch('/sales/summary', {
        type: args.type as string, startDate: args.start_date as string, endDate: args.end_date as string,
      });
      return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
    }
    if (name === 'get_inventory') {
      const data = await dFetch('/inventory/status', {
        baseDate: args.base_date as string,
    ...
    if (name === 'get_payroll') {
      const data = await dFetch('/hr/payroll', {
        payYearMo: args.pay_year_mo as string,
