...
const server = new Server({ name: 'hometax-mcp', version: '0.1.0' }, { capabilities: { tools: {} } });
const API_KEY = process.env.HOMETAX_API_KEY ?? '';
const BASE = 'https://api.hometax.go.kr/v1';
async function htFetch(path, params = {}) {
    const url = new URL(`${BASE}${path}`);
    url.searchParams.set('serviceKey', API_KEY);
    Object.entries(params).forEach(([k, v]) => url.searchParams.set(k, v));
    const res = await fetch(url.toString());
    if (!res.ok)
        throw new Error(`홈택스 API 오류: ${res.status} ${res.statusText}`);
    return res.json();
}
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    try {
        if (!API_KEY)
            throw new Error('HOMETAX_API_KEY 환경 변수가 설정되지 않았습니다.');
        if (name === 'verify_business_registration') {
            const data = await htFetch('/bizInfo', { bizNo: args.biz_no });
        ...
        if (name === 'get_received_invoices') {
            const data = await htFetch('/taxInvoice/received', {
                bizNo: args.biz_no, startDate: args.start_date, endDate: args.end_date,
            });
            return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
        }
        if (name === 'aggregate_vat_data') {
            const data = await htFetch('/vat/aggregate', { bizNo: args.biz_no, taxPeriod: args.tax_period });
            return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
        }
        if (name === 'get_year_end_tax_docs') {
            const data = await htFetch('/yearEndTax', { residentNo: args.resident_no, year: args.year });
            return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
        }
