...
const server = new Server({ name: 'publicdata-mcp', version: '0.1.0' }, { capabilities: { tools: {} } });
const API_KEY = process.env.PUBLIC_DATA_API_KEY ?? '';
async function dataFetch(url, params = {}) {
    const u = new URL(url);
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    try {
        if (!API_KEY)
            throw new Error('PUBLIC_DATA_API_KEY 환경 변수가 설정되지 않았습니다.');
        if (name === 'get_weather') {
            const data = await dataFetch('http://apis.data.go.kr/1360000/VilageFcstInfoService_2.0/getVilageFcst', { numOfRows: '100', pageNo: '1', dataType: 'JSON',
                base_date: args.base_date, base_time: args.base_time,
                nx: args.nx, ny: args.ny });
            return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
        }
        if (name === 'get_bus_arrival') {
            const data = await dataFetch('http://apis.data.go.kr/1613000/ArvlInfoInqireService/getSttnAcctoArvlPrearngeInfoList', { cityCode: args.city_code, nodeId: args.node_id });
            return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
        }
        if (name === 'get_subway_info') {
            const data = await dataFetch('http://swopenAPI.seoul.go.kr/api/subway/' + API_KEY + '/json/realtimePosition/0/50/' + encodeURIComponent(args.line_name));
            return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
        }
        if (name === 'get_real_estate_price') {
            const data = await dataFetch('http://apis.data.go.kr/1613000/RTMSDataSvcAptTradeDev/getRTMSDataSvcAptTradeDev', { LAWD_CD: args.lawd_cd, DEAL_YMD: args.deal_ymd, numOfRows: '100' });
            return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
        }
        if (name === 'get_population_stats') {
            const data = await dataFetch('http://apis.data.go.kr/1741000/regnPopService1/regnPopService1', { admCd: args.adm_cd, yearMonth: args.year_mo });
            return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
        }
